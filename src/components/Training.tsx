...
export default function Training() {
  return (
    <Section
      ...
      intro="Studio classes, courses and workshops."
    >
      <ol className="relative space-y-8 border-l border-white/10 pl-6 sm:pl-8">
        {workshops.map((w) => (
          <li key={`${w.title}-${w.organization}`} className="relative">
            <span
              aria-hidden
            ...
            <div className="flex flex-col gap-1 sm:flex-row sm:items-baseline sm:justify-between">
              ...
              {w.years ? <span className="text-sm text-star-dim">{w.years}</span> : null}
            </div>
            <p className="mt-1 text-sm text-star-dim">
              {[w.instructor, w.organization].filter(Boolean).join(" · ")}
            </p>
            {w.description ? (
